...
- [x] 创建 `packages/cli/src/fetcher.js`
- [x] 实现 `fetchTarball(source, ref)`：
...
- [x] 从响应头 `content-disposition` 中尝试提取 commit SHA（可选），若失败则为 null
- [x] 编写测试：
...
- [x] 实现 `extractTarball(tarballPath, manifest)`：
...
- [x] 实现 `cleanup(deletePaths)`，接受 `{ tarballPath, extractedDir }`：
...
- [x] 创建 `packages/cli/src/__tests__/fetcher.test.js`
- [x] 在描述性 `describe` 块中组织 3.1、3.2、3.3 的测试
- [x] mock 外部依赖（fetch、tar、fs）确保测试不联网、不写真实文件
- [x] 验证 `npx vitest run fetcher` 全绿