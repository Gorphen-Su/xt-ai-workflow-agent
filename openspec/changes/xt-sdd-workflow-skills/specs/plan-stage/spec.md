...
#### Scenario: 正常调用 writing-plans
- **WHEN** Bridge 转换完成且 Superpowers 可用
- **THEN** 调用 `superpowers:writing-plans`，计划文件保存到变更目录内的 `plan.md`

#### Scenario: writing-plans 降级
- **WHEN** Superpowers 不可用
- **THEN** 跳过 writing-plans，使用 Bridge 转换产出的 tasks.md 作为实现指导（无 checkbox 微步骤）

#### Scenario: 计划质量审查
- **WHEN** writing-plans 完成后
- **THEN** 系统 MUST 逐项检查编译约束遵守、import 正确性、无效代码、类型一致性，发现问题直接在计划文件中修复
