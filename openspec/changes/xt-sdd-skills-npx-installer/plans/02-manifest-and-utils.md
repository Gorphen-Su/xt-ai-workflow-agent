...
- [x] 创建 `packages/cli/src/manifest.js`（ESM），导出以下内容：
  ```js
  ...
  ```
- [x] 导出一个 `getManifestItems(manifest)` 辅助函数，展开所有清单路径为扁平文件列表（含 skills 内 SKILL.md 和 scripts/* 等所有文件）
- [x] 编写测试：验证 MANIFEST.skills 长度 = 6、每个 skill 名格式为 `xt-sdd-*`、MANIFEST 结构冻结（Object.freeze 或在测试中断言不应被修改）

### Step 2.2: 实现 errors.js

<!-- TODO: 统一错误类，便于调用方按 code 判断退出码 -->
- [x] 创建 `packages/cli/src/errors.js`，定义以下错误类：
...
- [x] 每个构造函数接受 `(code, message, options)`，`code` 为字符串（如 `'REF_NOT_FOUND'`）
- [x] 编写测试：验证 new FetcherError('NETWORK_ERROR', 'msg') 的 .code = 'NETWORK_ERROR'、.exitCode = 2、.message = 'msg'、instanceof Error 和 instanceof CliError

### Step 2.3: 实现 logger.js

<!-- TODO: 基于 kleur 的终端输出封装，保证一致的颜色语义 -->
- [x] 创建 `packages/cli/src/logger.js`，导出：
  - `info(msg)` — 白色（默认）
...
- [x] 所有函数写入 `process.stdout`（info/success/section/detail）或 `process.stderr`（warn/error）
- [x] 所有函数后面自动追加 `\n`
- [x] 编写测试：mock `process.stdout.write` 和 `process.stderr.write`，验证每种输出颜色前缀和流目标

### Step 2.4: 编写组合测试

- [x] 验证 manifest.js + errors.js + logger.js 可同时 import 无冲突
- [x] 验证 `npx vitest run` 通过 manifest/errors/logger 全部测试