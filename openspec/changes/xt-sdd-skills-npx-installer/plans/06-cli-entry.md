...
- [ ] 创建可执行脚本 `packages/cli/bin/xt-sdd-skills.js`，第一行 `#!/usr/bin/env node`
- [ ] 在入口立即执行 Node.js 版本检查：
  ```js
...
- [ ] 传递给子命令 handler：`{ command, tag, source, dryRun, noBackup }`
- [ ] 编写测试：
...
- [ ] 创建 `packages/cli/src/commands/install.js`，导出 `async function installCLI(options)`：
  ...
  9. process.exit(0)
- [ ] 所有步骤用 try/catch 包裹，catch 中统一错误处理

### Step 6.3: 实现 update 命令

<!-- TODO: update 子命令的完整流程编排，含备份 -->
- [ ] 创建 `packages/cli/src/commands/update.js`，导出 `async function updateCLI(options)`：
...
- [ ] 创建 `packages/cli/src/commands/list.js`，导出 `async function listCLI(options)`：
...
- [ ] 在 `bin/xt-sdd-skills.js` 层面实现 `async function main()` 并用 `.catch(handleError)` 兜底
- [ ] `handleError(err)`：
...
- [ ] 确保 `process.exit(0)` 只在正常路径被调用（不走 catch）

### Step 6.6: help/usage 输出

<!-- TODO: 用户友好帮助信息 -->
- [ ] 创建 `packages/cli/src/help.js`，导出 `printUsage()` 和 `printDetailedHelp(command)`：
...
- [ ] 无参数时调 `printUsage()`，未知子命令时 `printUsage()` + 错误

### Step 6.7: CLI 入口集成测试

<!-- TODO: 用 child_process 测试完整 CLI 入口 -->
- [ ] 创建 `packages/cli/src/__tests__/cli.test.js`
- [ ] 用 `execFile('node', [binPath, ...args])` 测试：
...
- [ ] install/update 集成测试留到步骤 7（端到端），这里只测 CLI 框架逻辑
- [ ] 验证 `npx vitest run cli` 全绿