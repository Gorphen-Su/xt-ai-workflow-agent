...
<!-- TODO: 完整目录复制，保证回滚能力 -->
- [x] 创建 `packages/cli/src/backup.js`
- [x] 实现 `createBackup(projectRoot, manifest, meta)`：
...
<!-- TODO: 提示用户清理过多备份 -->
- [x] 实现 `checkBackupCount(projectRoot)`：
...
- [x] 在 `packages/cli/src/__tests__/backup.test.js` 中组织所有测试
- [x] 每个测试需创建假的 `.claude/skills/` 目录结构（用 `fs.mkdtempSync` 或 `vitest` 的 `beforeEach`/`afterEach` 清理）
- [x] 验证 `npx vitest run backup` 全绿