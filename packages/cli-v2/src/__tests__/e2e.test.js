...
const MANIFEST = {
  skills: ['xt-sdd2-interview', 'xt-sdd2-draft'],
  templates: [
    {
  ...
  commands: ['xt-sdd2/interview', 'xt-sdd2/draft'],
};
...
async function buildFakeRepoTarball(version) {
  ...
  const files = {
    '.claude/skills/xt-sdd2-interview/SKILL.md': `# interview ${version}\nbody`,
    '.claude/skills/xt-sdd2-interview/references/deep/note.md': `# deep ${version}`,
    '.claude/skills/xt-sdd2-draft/SKILL.md': `# draft ${version}`,
    '.claude/commands/xt-sdd2/interview.md': `/interview ${version}`,
    '.claude/commands/xt-sdd2/draft.md': `/draft ${version}`,
...
describe('e2e: install → update 完整周期', () => {
  it('install → 验证 skill/command/template 全部正确写入（含子目录命令）', async () => {
    ...
    const skill = await fs.readFile(
      path.join(projectRoot, '.claude/skills/xt-sdd2-interview/SKILL.md'),
      'utf-8'
    );
    expect(skill).toContain('# interview v1');

    const deepRef = await fs.readFile(
      path.join(projectRoot, '.claude/skills/xt-sdd2-interview/references/deep/note.md'),
      'utf-8'
    );
    ...
    const cmd = await fs.readFile(
      path.join(projectRoot, '.claude/commands/xt-sdd2/interview.md'),
      'utf-8'
    );
    expect(cmd).toContain('/interview v1');
  ...
  it('install → update：第二次拉取覆盖 skill 并备份旧版', async () => {
    ...
    expect(r.backed_up).toEqual(
      expect.arrayContaining(['xt-sdd2-interview', 'xt-sdd2-draft'])
    );
    ...
    expect(newSkill).toContain('# interview v2');
    ...
    expect(newCmd).toContain('/interview v2');

    // 旧版仍在备份目录
    const backupSkill = await fs.readFile(
      path.join(r.backupDir, 'xt-sdd2-interview/SKILL.md'),
      'utf-8'
    );
    expect(backupSkill).toContain('interview v1');
