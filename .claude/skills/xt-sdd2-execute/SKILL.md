---
name: xt-sdd2-execute
description: Use when 用户说"开始实现"、"执行任务"、"TDD 开发"、"继续实现"，或 draft 冻结通过后的下一阶段触发。按冻结契约做测试驱动实现的入口。
---

# xt-sdd2 · execute — TDD 实现阶段
...
3. 冻结 delta 不可编辑：发现契约错误 → 按勘误协议处理，语义错误立即停手回 draft
...
- 语义疑问（哪怕它"明显不合理"）→ **停下**，汇报差异与两种可能的意图解读，请用户裁决后走 MODIFIED delta 回 draft 重冻
