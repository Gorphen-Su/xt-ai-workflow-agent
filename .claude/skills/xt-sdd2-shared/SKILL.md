...
- audit 报告绑定其运行的 HEAD hash；land 校验口径为「**生产代码路径相对审计 HEAD 的 diff 为空**」——审计报告自身及后续卷宗文档提交会使仓库 HEAD 前移，属预期事实而非新鲜度破坏；若 packages/** 等生产路径出现 diff 则报告作废须重审（2026-08-27 试点修正：原 HEAD 字面等值规则结构性不可行）
