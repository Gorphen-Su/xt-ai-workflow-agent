---
name: xt-sdd2-verify
description: Use when 用户说"审计"、"出审计报告"、"审查实现"、"验证闭环"，或 apply 完成后的下一阶段，或使用 /xt-sdd2:verify 时触发。三方闭环校验与放行判定的唯一出口。
---
...
6. FAIL 的缺口修复后 MUST 整体重跑 verify 生成新报告替换旧文件
...
### 5. 分流
- PASS → 提示 `/xt-sdd2:archive`
- FAIL → 列缺口清单等待整改；整改完成（通常回 apply 局部任务组）后整体重跑本阶段
...
| "补录映射表然后标'整改后通过'，下午要用" | 补录可以（verify 阶段允许整改循环），但结论必须是新一份整体重跑后的 PASS——协商态措辞会让评审会上的人误以为它本来就闭环 |
